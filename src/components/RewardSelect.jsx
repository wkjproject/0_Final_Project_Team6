...
const RewardSelect = () => {
  ...
  const calculateTotalAmount = () => {
    return selectedRewards.reduce((total, reward) => total + reward.projRewardAmount, 0);
  };
    ...
    <div className="backgroundArea" ref={backgroundAreaRef}>
                ...
                <li key={project.proj_id}>
                  {project.projReward.map((reward, index) => (
                    <div key={index}>
                      ...
                      {projectIndex !== ProjectData.length - 1 || index !== project.projReward.length - 1 ? <hr /> : null}
                    </div>
                  ))}
                </li>
              ))}
        ...
        <div>
          ...
          <ul className="no-bullets2">
            {selectedRewards.map((selectedReward, index) => (
              <li key={index} className={index >= 0 && index <= 100 ? "second-table" : ""}>
                <table>
                  <tr>
                    <td>
                      리워드
                  ...
                  <tr>
                    <td>금액</td>
                    <td>: {formatAmount(selectedReward.projRewardAmount)} 원</td>
                  </tr>
                  <tr>
                    <td>갯수</td>
      ...
      <div className="button-container">
        ...
        <div className="button-group">
          <button className={`heartBtn ${heartClicked ? 'clicked' : ''}`} onClick={toggleHeart}>
            {heartClicked ? '❤️' : '🤍'} {heartClicked ? clickedCount : ''}
          </button>
          <button className="shareBtn" style={{ marginLeft: '20px' }}>공유하기</button>
        </div>
      </div>
    </div>
  );
